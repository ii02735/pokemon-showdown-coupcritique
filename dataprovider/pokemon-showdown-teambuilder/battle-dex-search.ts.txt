...
import {Moves as BattleMovedex} from "../../data/moves";
import {Pokedex as BattlePokedex} from "../../data/pokedex";
import {TypeName} from "smogon";
...
export class DexSearch {
	 ...
	 */
	constructor(private dex: ModdedDex, searchType: SearchType | '' = '', formatid = '' as ID, species = '' as ID) {
	...
	textSearch(query: string): SearchRow[] {
		...
		const searchType: SearchType | '' = this.typedSearch?.searchType || '';
		...
		const searchTypeIndex = (searchType ? DexSearch.typeTable[searchType] : -1);
		...
		const searchPasses: SearchPass[] = [['normal', i, query]];
		...
		if (query in BattleAliases) {
			if (['sub', 'tr'].includes(query) || !toID(BattleAliases[query]).startsWith(query)) {
				...
				const aliasPassType: SearchPassType = (queryAlias === 'hiddenpower' ? 'exact' : 'normal');
		...
		if (!this.exactMatch && BattleSearchIndex[i][0].substr(0, query.length) !== query) {
			...
			const matchQuery = query.substr(0, matchLength);
		...
		const instafilterSort = [0, 1, 2, 5, 4, 3, 6, 7, 8];
		...
		for (i = 0; i < BattleSearchIndex.length; i++) {
			if (!passType) {
				const searchPass = searchPasses.shift();
				if (!searchPass) break;
			...
			const entry = BattleSearchIndex[i];
			let id = entry[0];
			const type = entry[1];
			...
			const curBufLength = (passType === 'alias' && bufs[typeIndex].length);
	...
	private instafilter(searchType: SearchType | '', fType: SearchType, fId: ID): SearchRow[] {
		const buf: SearchRow[] = [];
		const illegalBuf: SearchRow[] = [];
		const illegal = this.typedSearch?.illegalReasons;
		if (searchType === 'pokemon') {
			switch (fType) {
			case 'type':
				const type = fId.charAt(0).toUpperCase() + fId.slice(1) as TypeName;
				...
				break;
			case 'ability':
				const ability = Dex.abilities.get(fId).name;
			...
			switch (fType) {
			case 'type':
				const type = fId.charAt(0).toUpperCase() + fId.slice(1);
				...
				break;
			case 'category':
				const category = fId.charAt(0).toUpperCase() + fId.slice(1);
...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected canLearn(speciesid: ID, moveid: ID) {
		...
		while (learnsetid) {
			...
			if (this.formatType?.startsWith('bdsp')) table = table['gen8bdsp'];
			if (this.formatType === 'letsgo') table = table['gen7letsgo'];
			const learnset = table.learnsets[learnsetid];
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		const slices: {[k: string]: number} = table.formatSlices;
		if (format === 'ubers' || format === 'uber') { tierSet = tierSet.slice(slices.Uber); } else if (isVGCOrBS) {
		...
		} else if (format === 'ou') { tierSet = tierSet.slice(slices.OU); } else if (format === 'uu') { tierSet = tierSet.slice(slices.UU); } else if (format === 'ru') { tierSet = tierSet.slice(slices.RU || slices.UU); } else if (format === 'nu') { tierSet = tierSet.slice(slices.NU || slices.RU || slices.UU); } else if (format === 'pu') { tierSet = tierSet.slice(slices.PU || slices.NU); } else if (format === 'zu') { tierSet = tierSet.slice(slices.ZU || slices.PU || slices.NU); } else if (format === 'lc' || format === 'lcuu' || format.startsWith('lc') || (format !== 'caplc' && format.endsWith('lc'))) { tierSet = tierSet.slice(slices.LC); } else if (format === 'cap') { tierSet = tierSet.slice(0, slices.AG || slices.Uber).concat(tierSet.slice(slices.OU)); } else if (format === 'caplc') {
	...
	filter(row: SearchRow, filters: string[][]) {
		...
		for (const [filterType, value] of filters) {
			...
			case 'tier':
				...
				break;
			case 'ability':
				if (!this.dex.hasAbility(species, value)) return false;
				break;
			case 'move':
...
class BattleAbilitySearch extends BattleTypedSearch<'ability'> {
	...
	getBaseResults() {
		...
		if (isAAA || format.includes('metronomebattle') || isHackmons) {
			...
			const goodAbilities: SearchRow[] = [['header', "Abilities"]];
			const poorAbilities: SearchRow[] = [['header', "Situational Abilities"]];
			const badAbilities: SearchRow[] = [['header', "Unviable Abilities"]];
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		while (learnsetid) {
			const learnset = lsetTable.learnsets[learnsetid];
			if (learnset) {
				for (const moveid in learnset) {
					const learnsetEntry = learnset[moveid];
	...
	sort(results: SearchRow[], sortCol: string, reverseSort?: boolean): SearchRow[] {
		...
		switch (sortCol) {
		case 'power':
			const powerTable: {[id: string]: number | undefined} = {
			...
			return results.sort(([rowType1, id1], [rowType2, id2]) => {
				const move1 = this.dex.moves.get(id1);
				const move2 = this.dex.moves.get(id2);
				const pow1 = move1.basePower || powerTable[id1] || (move1.category === 'Status' ? -1 : 1400);
				const pow2 = move2.basePower || powerTable[id2] || (move2.category === 'Status' ? -1 : 1400);
		...
		case 'pp':
			return results.sort(([rowType1, id1], [rowType2, id2]) => {
				const pp1 = this.dex.moves.get(id1).pp || 0;
				const pp2 = this.dex.moves.get(id2).pp || 0;
